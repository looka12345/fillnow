<?php
 class Counselling_management extends CI_Model
 {
 ...
 function insertData($data) 
{
	if($data)
	{
		$this->db->insert('counselling',$data);
		return 1;
...
function getSingalData($counselling_id=FALSE) 
{
	$this->db->select('upe.*');
	$this->db->from('counselling AS upe');
	$this->db->where('upe.counselling_id',$counselling_id);
	$query = $this->db->get();
	 if($query->num_rows()>0) 
	   {
		$checkQuery = $query->result();
...
function updateData($data,$bmi_id) 
{
	...
	$this->db->where('counselling_id', $bmi_id);
	if($this->db->update('counselling',$data))
	{
		return 1;
