<?php
 class Counter_management extends CI_Model
 {
...
function getSingalData($counters_id=FALSE) 
{
	$this->db->select('upe.*');
	$this->db->from('counters AS upe');
	$this->db->where('upe.counters_id',$counters_id);
	$query = $this->db->get();
	 if($query->num_rows()>0) 
	   {
		$checkQuery = $query->result();
...
function updateData($data,$bmi_id) 
{
	...
	$this->db->where('counters_id', $bmi_id);
	if($this->db->update('counters',$data))
	{
		return 1;
