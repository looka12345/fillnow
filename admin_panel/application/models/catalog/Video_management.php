<?php
 class Video_management extends CI_Model
 {
...
function getSingalData($video_id=FALSE) 
{
	$this->db->select('upe.*');
	$this->db->from('video AS upe');
	$this->db->where('upe.video_id',$video_id);
	$query = $this->db->get();
	 if($query->num_rows()>0) 
	   {
		$checkQuery = $query->result();
