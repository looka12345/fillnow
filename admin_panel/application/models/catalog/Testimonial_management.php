<?php
class Testimonial_management extends CI_Model
{
...
function getSingalData($testimonial_id=FALSE)
{
	$this->db->select('upe.*');
	$this->db->from('testimonial AS upe');
	$this->db->where('upe.testimonial_id',$testimonial_id);
	$query = $this->db->get();
	if($query->num_rows()>0)
	{
		$checkQuery = $query->result();
...
function updateData($data,$bmi_id)
{
	...
	$this->db->where('testimonial_id', $bmi_id);
