<?php
 class Placement_management extends CI_Model
 {
...
function getSingalData($placement_id=FALSE) 
{
	$this->db->select('upe.*');
	$this->db->from('placement AS upe');
	$this->db->where('upe.placement_id',$placement_id);
	$query = $this->db->get();
	 if($query->num_rows()>0) 
	   {
		$checkQuery = $query->result();
...
function updateData($data,$bmi_id) 
{
	...
	$this->db->where('placement_id', $bmi_id);
	if($this->db->update('placement',$data))
	{
		return 1;
