	...
	<header class="page-header">
		...
		<div class="right-wrapper pull-right">
			<ol class="breadcrumbs">
				...
				<li><span>Gallery Management</span></li>
	...
	<section class="panel">
		...
		<div class="panel-body">
			...
			<tbody>
				...
				if(isset($gallery_arr) && !empty($gallery_arr)) { ?>
				<?php foreach($gallery_arr as $value) { ?>
				<tr  role="row">
					<td class="sorting_1"><?php echo ucfirst($value->name); ?></td>
					<td class="sorting_1"><?php if(isset($value->image) && !empty($value->image)) { ?> <img src="<?php echo site_url("../assets/sitesfile/image/gallery")."/".$value->image;?>" height="100" width="100" /><?php } ?></td>
					<td class="actions">
						<?php if($value->active_status==1){ ?>
						<img src="<?php echo site_url("assets/images/icons")."/active.png"; ?>" onclick="changeStatus(<?php echo $value->gallery_id; ?>,'gallery',1)" title="status" />
						<?php }else{ ?>
						<img src="<?php echo site_url("assets/images/icons")."/deactive.png"; ?>" onclick="changeStatus(<?php echo $value->gallery_id; ?>,'gallery',0)" title="status" />
						<?php } ?>
						<a class="on-default edit-row" href="<?php echo site_url("catalog/gallery/edit")."/".$value->gallery_id 	; ?>"><i class="fa fa-pencil"></i></a>
						<a class="on-default remove-row" onclick="recordAddToTrace(this,'<?php echo $value->gallery_id 	 ?>','gallery','<?php echo $value->delete_status?>','1')"><i class="fa fa-trash-o"></i></a>
						</td><td>	<input type="text"  id="priority<?php echo $value->gallery_id ?>" size="1" value="<?=$value->priority?>" />
						<input type="button" id="save" onclick="setpriority('priority<?php echo $value->gallery_id ?>',<?php echo $value->gallery_id ?>,'gallery')"  value="Update" />			</td>
					</tr>
					<?php } } else { ?>
