<section class="content-body" role="main">
	<header class="page-header">
		...
		<div class="right-wrapper pull-right">
			<ol class="breadcrumbs">
				...
				<li><span>Testimonial Management</span></li>
	...
	<section class="panel">
		<header class="panel-heading">
			...
			<h2 class="panel-title">Testimonial Management</h2>
		...
		<div class="panel-body">
			...
			<tbody>
				...
				if(isset($testimonial_arr) && !empty($testimonial_arr)) { ?>
				<?php foreach($testimonial_arr as $value) { ?>
				<tr  role="row">
					<td class="sorting_1"><?php echo $value->name; ?></td>
					<td class="actions">
						<?php if($value->active_status==1){ ?>
						<img src="<?php echo site_url("assets/images/icons")."/active.png"; ?>" onclick="changeStatus(<?php echo $value->testimonial_id; ?>,'testimonial',1)" title="status" />
						<?php }else{ ?>
						<img src="<?php echo site_url("assets/images/icons")."/deactive.png"; ?>" onclick="changeStatus(<?php echo $value->testimonial_id; ?>,'testimonial',0)" title="status" />
						<?php } ?>
						<a class="on-default edit-row" href="<?php echo site_url("catalog/testimonialmanage/edit")."/".$value->testimonial_id 	; ?>"><i class="fa fa-pencil"></i></a>
						<a class="on-default remove-row" onclick="recordAddToTrace(this,'<?php echo $value->testimonial_id 	 ?>','testimonial','<?php echo $value->delete_status?>','1')"><i class="fa fa-trash-o"></i></a>
						</td><td>	<input type="text"  id="priority<?php echo $value->testimonial_id ?>" size="1" value="<?=$value->priority?>" />
						<input type="button" id="save" onclick="setpriority('priority<?php echo $value->testimonial_id ?>',<?php echo $value->testimonial_id ?>,'testimonial')"  value="Update" />			</td>
					</tr>
					<?php } } else { ?>
