<section class="content-body" role="main">
					...
					<section class="panel">
						<header class="panel-heading">
							...
							<h2 class="panel-title">Blog Management</h2>
					...
					if(isset($blogpost_arr) && !empty($blogpost_arr)) { ?>
									<?php foreach($blogpost_arr as $value) { ?>
										<tr  role="row">
										<td class="sorting_1"><?php echo ucfirst($value->name); ?></td>
										<td class="sorting_1"><?php if(isset($value->image) && !empty($value->image)) { ?> <img src="<?php echo site_url("../assets/sitesfile/image/blogpost")."/".$value->image;?>" height="100" width="100" /><?php } ?></td>

										<td class="actions">
											<?php if($value->active_status==1){ ?>
											<img src="<?php echo site_url("assets/images/icons")."/active.png"; ?>" onclick="changeStatus(<?php echo $value->blogpost_id; ?>,'blogpost',1)" title="status" />
											<?php }else{ ?> 
										   	<img src="<?php echo site_url("assets/images/icons")."/deactive.png"; ?>" onclick="changeStatus(<?php echo $value->blogpost_id; ?>,'blogpost',0)" title="status" />
											<?php } ?>
											<a class="on-default edit-row" href="<?php echo site_url("catalog/blogpost/edit")."/".$value->blogpost_id 	; ?>"><i class="fa fa-pencil"></i></a>
											<a class="on-default remove-row" onclick="recordAddToTrace(this,'<?php echo $value->blogpost_id 	 ?>','blogpost','<?php echo $value->delete_status?>','1')"><i class="fa fa-trash-o"></i></a>
							</td><td>	<input type="text"  id="priority<?php echo $value->blogpost_id ?>" size="1" value="<?=$value->priority?>" />
							<input type="button" id="save" onclick="setpriority('priority<?php echo $value->blogpost_id ?>',<?php echo $value->blogpost_id ?>,'blogpost')"  value="Update" />			</td>
									</tr>
									<?php } } else { ?>
