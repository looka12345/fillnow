...
class Segment extends MY_Controller
{
    ...
    public function customer_segment_list()
    {

        $admin_sess_data = $this->session->userdata('admin_sess_data');
        $res = $this->DashboardModel->getAllList('segment', 'id', 'asc');
        ...
        $this->load->helper('custom1');
        $this->render("admin/segment_list");
    }
    ...
    public function saveSegmentData()
    {
        ...
        $this->form_validation->set_rules('minimum_purchase', 'minimum purchase', 'trim|required');
        $this->form_validation->set_rules('maximum_purchase', 'maximum purchase', 'trim|required');
        if ($this->form_validation->run() == false) {
            ...
            $user_data = array(
                ...
                "status" => 1,
                "minimum_purchase" => $pdata['minimum_purchase'],
                "maximum_purchase" => $pdata['maximum_purchase'],
            );
            $res = $this->DashboardModel->saveAllInsert($user_data, 'segment');
            if ($res) {
                $admin_sess_data = $this->session->userdata('admin_sess_data');
    ...
    public function editSegmentList()
    {
        $getData = $this->input->get();
        $edit_id = decryptor($getData['id']);
        $this->data['edit_content'] = $this->DashboardModel->getAllEditDetails($edit_id, 'segment', 'id');
        $this->render('admin/edit_segment');
    }
    ...
    public function updateSegmentData()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $res = $this->DashboardModel->updateAll($user_data, $edit_id, 'segment', 'id');
            ...
            if ($res) {
                ...
                $log_data = array(
                    "login_mast_id" => $login_mast_id,
                    "ip" => $this->input->ip_address(),
                    "message" => "Segment Updated by " . getname($login_mast_id),
                );
                writelog($log_data);
                $responce['err'] = 0;
                $responce['msg'] = "The Data has been successfully saved.";
