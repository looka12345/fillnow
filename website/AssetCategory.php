...
class AssetCategory extends MY_Controller
{
    ...
    public function asset_category_list()
    {
        ...
        $res = $this->DashboardModel->getAllList('asset_category', 'id', 'asc');
        ...
        $this->load->helper('custom1');
        $this->render("admin/asset_category_list");
    }
    ...
    public function addAssetCategory()
    {
        ...
        $this->render('admin/add_asset_category');
    }

    public function saveAssetCategoryData()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $user_data = array(
                "category_name" => trim($pdata['name']),
                "status" => 1,
            );
            $res = $this->DashboardModel->saveAllInsert($user_data, 'asset_category');
            if ($res) {
                $admin_sess_data = $this->session->userdata('admin_sess_data');
                ...
                $log_data = array(
                    ...
                    "message" => "Asset Category Added by " . getname($login_mast_id),
                );
                writelog($log_data);
                $responce['err'] = 0;
    ...
    public function editAssetCategoryList()
    {
        $getData = $this->input->get();
        $edit_id = decryptor($getData['id']);
        $this->data['edit_content'] = $this->DashboardModel->getAllEditDetails($edit_id, 'asset_category', 'id');
        $this->render('admin/edit_asset_category');
    }

    public function updateAssetCategoryData()
    {
        $pdata = $this->input->post();
        $this->load->library('form_validation');
        $this->form_validation->set_rules('name', 'name', 'trim|required');
        ...
        if ($this->form_validation->run() == false) {
            ...
            $res = $this->DashboardModel->updateAll($user_data, $edit_id, 'asset_category', 'id');
