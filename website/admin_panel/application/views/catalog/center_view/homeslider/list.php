<section class="content-body" role="main">
    <header class="page-header">
        ...
        <div class="right-wrapper pull-right">
            <ol class="breadcrumbs">
                ...
                <li><span>App Feature Management</span></li>
            </ol>
    ...
    <section class="panel">
        <header class="panel-heading">
            ...
            <h2 class="panel-title">App Feature Management</h2>
        ...
        <div class="panel-body">
            ...
            <div id="datatable-editable_wrapper" class="dataTables_wrapper no-footer"><div class="row datatables-header form-inline"><div class="col-sm-12 col-md-6"><div class="dataTables_length" id="datatable-editable_length"></div></div><div class="col-sm-12 col-md-6"><div id="datatable-editable_filter" class="dataTables_filter"><form id="SearchForm" method="post"><label><input type="search" class="form-control" name="searchhomeslider" placeholder="App Feature" value="<?php echo $searchhomeslider; ?>" aria-controls="datatable-editable"></label><label><input type="button"  onclick='javascript:searchload("<?php echo site_url("catalog/homeslidermanage"); ?>")'  style="  background: none repeat scroll 0 0 #491e03;color: #fff;width:50%;margin-left:25%;"  value="Search" /></label></form></div></div></div><div class="table-responsive"><table id="datatable-editable" class="table table-bordered table-striped mb-none dataTable no-footer" role="grid" aria-describedby="datatable-editable_info">
                        ...
                        <tbody>
                            <?php if (isset($homeslider_arr) && !empty($homeslider_arr)) { ?>
                                <?php foreach ($homeslider_arr as $value) { ?>
                                    <tr  role="row">
                                        <td class="sorting_1"><?php echo ucfirst($value->name); ?></td>

                                        <td class="actions">
                                          <?php if ($value->active_status == 1) { ?>
                                                <img src="<?php echo site_url("assets/images/icons") . "/active.png"; ?>" onclick="changeStatus(<?php echo $value->home_slider_id; ?>, 'home_slider', 1)" title="status" />
                                            <?php } else { ?> 
                                                <img src="<?php echo site_url("assets/images/icons") . "/deactive.png"; ?>" onclick="changeStatus(<?php echo $value->home_slider_id; ?>, 'home_slider', 0)" title="status" />
                                            <?php } ?>
                                            <a class="on-default edit-row" href="<?php echo site_url("catalog/homeslidermanage/edit") . "/" . $value->home_slider_id; ?>"><i class="fa fa-pencil"></i></a>
                                            <a class="on-default remove-row" onclick="recordAddToTrace(this, '<?php echo $value->home_slider_id ?>', 'home_slider', '<?php echo $value->delete_status ?>', '1')"><i class="fa fa-trash-o"></i></a>
                                        </td><td>	<input type="text"  id="priority<?php echo $value->home_slider_id ?>" size="1" value="<?= $value->priority ?>" />
                                            <input type="button" id="save" onclick="setpriority('priority<?php echo $value->home_slider_id ?>',<?php echo $value->home_slider_id ?>, 'home_slider')"  value="Update" />			</td>
                                    </tr>
                             <?php }
                               } else { ?>
