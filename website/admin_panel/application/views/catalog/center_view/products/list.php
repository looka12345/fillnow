<section class="content-body" role="main">
  <header class="page-header">
    ...
    <div class="right-wrapper pull-right">
      <ol class="breadcrumbs">
        ...
        <li><span>Product & Service Management</span></li>
  ...
  <section class="panel">
    <header class="panel-heading">
      ...
      <h2 class="panel-title">Product & Service Management</h2>
    ...
    <div class="panel-body">
      <div class="row">
        <div class="col-sm-6">
          <div class="mb-md">
            <a href="<?php echo site_url("catalog/products/add"); ?>" >Add</a> <i class="fa fa-plus"></i>
          </div>
        </div>
      </div>
      <div id="datatable-editable_wrapper" class="dataTables_wrapper no-footer"><div class="row datatables-header form-inline"><div class="col-sm-12 col-md-6"><div class="dataTables_length" id="datatable-editable_length"></div></div><div class="col-sm-12 col-md-6"><div id="datatable-editable_filter" class="dataTables_filter"><form id="SearchForm" method="post"><label><input type="search" class="form-control" name="searchproducts" placeholder="Product & Service" value="<?php echo $searchproducts; ?>" aria-controls="datatable-editable"></label><label><input type="button"  onclick='javascript:searchload( "<?php echo site_url("catalog/products"); ?>")'  style="  background: none repeat scroll 0 0 #491e03;color: #fff;width:50%;margin-left:25%;"  value="Search" /></label></form></div></div></div><div class="table-responsive"><table id="datatable-editable" class="table table-bordered table-striped mb-none dataTable no-footer" role="grid" aria-describedby="datatable-editable_info">
      <thead>
        <tr role="row"><th class="sorting_asc" tabindex="0" aria-controls="datatable-editable" rowspan="1" colspan="1" style="width: 205px;" aria-sort="ascending" aria-label="Rendering engine: activate to sort column ascending">Name</th><th class="sorting_asc" tabindex="0" aria-controls="datatable-editable" rowspan="1" colspan="1" style="width: 205px;" aria-sort="ascending" aria-label="Rendering engine: activate to sort column ascending">Taxonomy</th><th class="sorting_disabled" rowspan="1" colspan="1" style="width: 98px;" aria-label="Actions">Actions</th><th class="sorting_disabled" rowspan="1" colspan="1" style="width: 98px;" aria-label="Actions">Priority</th></tr>
      </thead>
      <tbody>
        ...
        <tr  role="row">
          ...
          <td class="sorting_1"><?php echo $value->taxonomy; ?></td>
          
          <td class="actions">
            <?php if($value->active_status==1){ ?>
            <img src="<?php echo site_url("assets/images/icons")."/active.png"; ?>" onclick="changeStatus(<?php echo $value->products_id; ?>,'products',1)" title="status" />
            <?php }else{ ?>
            <img src="<?php echo site_url("assets/images/icons")."/deactive.png"; ?>" onclick="changeStatus(<?php echo $value->products_id; ?>,'products',0)" title="status" />
            <?php } ?>
            <a class="on-default edit-row" href="<?php echo site_url("catalog/products/edit")."/".$value->products_id   ; ?>"><i class="fa fa-pencil"></i></a>
            <a class="on-default remove-row" onclick="recordAddToTrace(this,'<?php echo $value->products_id    ?>','products','<?php echo $value->delete_status?>','1')"><i class="fa fa-trash-o"></i></a>
            </td><td> <input type="text"  id="priority<?php echo $value->products_id ?>" size="1" value="<?=$value->priority?>" />
            <input type="button" id="save" onclick="setpriority('priority<?php echo $value->products_id ?>',<?php echo $value->products_id ?>,'products')"  value="Update" />     </td>
          </tr>
          <?php } } else { ?>
