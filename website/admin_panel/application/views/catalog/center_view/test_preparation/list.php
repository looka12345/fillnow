<section class="content-body" role="main">
					<header class="page-header">
						<h2>Test Preparation Management</h2>
						...
						<div class="right-wrapper pull-right">
							<ol class="breadcrumbs">
								...
								<li><span>Test Preparation Management</span></li>
					...
					<section class="panel">
						<header class="panel-heading">
							...
							<h2 class="panel-title">Test Preparation Management</h2>
					...
					if(isset($test_preparation_arr) && !empty($test_preparation_arr)) { ?>
									<?php foreach($test_preparation_arr as $value) { ?>
										<tr  role="row">
										<td class="sorting_1"><?php echo ucfirst($value->name); ?></td>
										<td class="sorting_1"><?php if(isset($value->image) && !empty($value->image)) { ?> <img src="<?php echo site_url("../assets/sitesfile/image/test_preparation")."/".$value->image;?>" height="100" width="100" /><?php } ?></td>

										<td class="actions">
											<?php if($value->active_status==1){ ?>
											<img src="<?php echo site_url("assets/images/icons")."/active.png"; ?>" onclick="changeStatus(<?php echo $value->test_preparation_id; ?>,'test_preparation',1)" title="status" />
											<?php }else{ ?> 
										   	<img src="<?php echo site_url("assets/images/icons")."/deactive.png"; ?>" onclick="changeStatus(<?php echo $value->test_preparation_id; ?>,'test_preparation',0)" title="status" />
											<?php } ?>
											<a class="on-default edit-row" href="<?php echo site_url("catalog/test_preparation/edit")."/".$value->test_preparation_id 	; ?>"><i class="fa fa-pencil"></i></a>
											<a class="on-default remove-row" onclick="recordAddToTrace(this,'<?php echo $value->test_preparation_id 	 ?>','test_preparation','<?php echo $value->delete_status?>','1')"><i class="fa fa-trash-o"></i></a>
							</td><td>	<input type="text"  id="priority<?php echo $value->test_preparation_id ?>" size="1" value="<?=$value->priority?>" />
							<input type="button" id="save" onclick="setpriority('priority<?php echo $value->test_preparation_id ?>',<?php echo $value->test_preparation_id ?>,'test_preparation')"  value="Update" />			</td>
									</tr>
									<?php } } else { ?>
