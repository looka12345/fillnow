<?php
 class Blogpost_manage extends CI_Model
 {
...
function getSingalData($blogpost_id=FALSE) 
{
	$this->db->select('upe.*');
	$this->db->from('blogpost AS upe');
	$this->db->where('upe.blogpost_id',$blogpost_id);
	$query = $this->db->get();
	 if($query->num_rows()>0) 
	   {
		$checkQuery = $query->result();
...
function updateData($data,$bmi_id) 
{
	...
	$this->db->where('blogpost_id', $bmi_id);
