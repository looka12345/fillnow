<?php
 class Act_management extends CI_Model
 {
...
function getSingalData($act_id=FALSE) 
{
	$this->db->select('upe.*');
	$this->db->from('act AS upe');
	$this->db->where('upe.act_id',$act_id);
	$query = $this->db->get();
	 if($query->num_rows()>0) 
	   {
		$checkQuery = $query->result();
...
function updateData($data,$bmi_id) 
{
	...
	$this->db->where('act_id', $bmi_id);
	if($this->db->update('act',$data))
	{
		return 1;
